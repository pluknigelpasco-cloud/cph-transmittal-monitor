...
import { Search, Printer, Trash2, ArrowUpDown, Check, RefreshCw, Calendar } from 'lucide-react';
          ...
          <div className="flex items-center gap-2">
            {isNoticeModule && (
              <button
                ...
                className="px-4 py-2 rounded-xl bg-brand-blue hover:bg-navy text-white text-xs font-bold shadow-md shadow-brand-blue/20 transition-all disabled:opacity-40 flex items-center gap-1.5 cursor-pointer"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 pt-2">
          <div className="sm:col-span-6 relative">
            <input
              ...
              placeholder="Search patient name, series number, deficiency…"
            ...
            <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 text-[10px] font-black uppercase text-slate-500 z-10">
              <tr>
                ...
                <th className="py-3 px-4">
                  <button onClick={() => toggleSort('reference')} className="flex items-center gap-1">
                    Reference / Series
                ...
                {isNoticeModule && <th className="py-3 px-4 text-right">Claim Amount</th>}
                <th className="py-3 px-4">
                  <button onClick={() => toggleSort('baseDate')} className="flex items-center gap-1">
                    {module === 'RTH' || module === 'DENIED'
                      ? 'Notice Date'
                      : module === 'INPATIENT'
                      ? 'Discharge Date'
                      : 'Encounter Date'}
                ...
                {isNoticeModule && <th className="py-3 px-4">Transmitter</th>}
                <th className="py-3 px-4">Checklist Actions</th>
                {user.role === 'ADMIN' && isNoticeModule && <th className="py-3 px-4 text-right">Delete</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-xs">
              {records.length === 0 ? (
                ...
                records.map(r => {
                  ...
                  return (
                    <tr key={r.id} className={`hover:bg-blue-50/30 transition-colors ${isSelected ? 'bg-blue-50/50' : ''}`}>
                      {isNoticeModule && (
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          {(module === 'INPATIENT' || module === 'HD') && (
                            ...
                            >
                              {r.completed ? '✓ Transmitted' : 'Transmit'}
                            </button>
                          )}
                        </div>
                      </td>
